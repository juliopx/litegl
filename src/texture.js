...
var Texture = GL.Texture = function (width, height, options, gl) {
	...
	if(( (this.minFilter != gl.NEAREST && this.minFilter != gl.LINEAR) || this.wrapS != gl.CLAMP_TO_EDGE || this.wrapT != gl.CLAMP_TO_EDGE) && (!utils.isPowerOfTwo(this.width) || !utils.isPowerOfTwo(this.height)))
	{
	...
	if(width && height)
	{
		...
		if(this.texture_type == gl.TEXTURE_2D)
		{
			...
			if (utils.isPowerOfTwo(width) && utils.isPowerOfTwo(height) && options.minFilter && options.minFilter != gl.NEAREST && options.minFilter != gl.LINEAR) {
...
Texture.prototype.drawTo = function(callback, params)
{
	...
	{
		if( utils.getTime() - this.time >= 1000*60 )
		{
...
* @param {Shader} [shader=null] optional shader to apply while copying
*/
...
Texture.fromImage = function(image, options) {
	...
	if (utils.isPowerOfTwo(texture.width) && utils.isPowerOfTwo(texture.height) && options.minFilter && options.minFilter != gl.NEAREST && options.minFilter != gl.LINEAR) {
		texture.bind();
		gl.generateMipmap(texture.texture_type);
...
/**
* Create a generative texture from a shader ( must Shader.getScreenShader as reference for the shader )
...
Texture.cubemapFromImage = function( image, options ) {
	...
	for(var i = 0; i < 6; i++)
	{
		var canvas = utils.createCanvas( size, size );
